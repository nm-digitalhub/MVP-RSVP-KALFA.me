                ...
                >
                    ...
                    <span class="min-w-0 flex-1">
                        <span class="flex flex-wrap items-center gap-1.5 sm:gap-2">
                            <span class="truncate text-sm font-medium tracking-tight text-slate-900">{{ $label }}</span>
                            <?php if ($count > 0): ?>
                                <span class="inline-flex shrink-0 items-center rounded-sm px-1.5 py-0.5 text-[11px] {{ $countClass }}">{{ $count }}</span>
                            <?php endif; ?>
                            <?php if ($contextLabel): ?>
                                <span class="hidden items-center rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] sm:inline-flex {{ $badgeClass }}">{{ $contextLabel }}</span>
                            <?php endif; ?>
                        </span>
                        <span class="mt-0.5 hidden text-xs text-slate-500 sm:block">{{ $description ?: trans_choice('{0} No items|{1} :count item|[2,*] :count items', $count, ['count' => number_format($count)]) }}</span>
                    </span>
                </button>

                <?php if ($addAction && $addLabel): ?>
                    <button
                        type="button"
                        wire:click.stop="$dispatch('{{ $addAction }}')"
                        class="inline-flex min-h-9 shrink-0 items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:border-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand/40"
                    >
