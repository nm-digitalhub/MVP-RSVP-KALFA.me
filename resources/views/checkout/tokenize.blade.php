...
<body class="font-sans text-gray-900 max-w-[420px] mx-auto my-8 px-4">
    ...
    <form id="checkout-form" data-og="form" method="post" action="#" class="mt-6 space-y-4">
        ...
        <x-ts-input
            ...
            dusk="checkout-card-number"
            dir="ltr"
        />
        <x-ts-select.native id="og-expmonth" name="og-expmonth" label="חודש תפוגה *" required dir="ltr" dusk="checkout-exp-month">
            <option value="">חודש</option>
            @for($i = 1; $i <= 12; $i++)
                <option value="{{ str_pad((string)$i, 2, '0', STR_PAD_LEFT) }}">{{ str_pad((string)$i, 2, '0', STR_PAD_LEFT) }}</option>
            @endfor
        </x-ts-select.native>
        <x-ts-select.native id="og-expyear" name="og-expyear" label="שנת תפוגה *" required dir="ltr" dusk="checkout-exp-year">
        ...
        <button type="submit" id="pay-btn" class="w-full min-h-[48px] px-4 py-3 bg-brand text-white text-base font-medium rounded-xl cursor-pointer hover:bg-brand-hover focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 disabled:opacity-60 disabled:cursor-not-allowed">
