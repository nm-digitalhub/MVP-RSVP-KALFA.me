<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16" role="main" aria-label="{{ __('Organization management') }}">
    @if(session('error'))
        <div class="p-4 rounded-xl bg-red-50/90 border border-red-200/60 text-red-700 text-sm" role="alert">
            <div class="flex items-start gap-2">
                <svg class="w-5 h-5 flex-shrink-0 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    @if(session('success'))
        <div class="p-4 rounded-xl bg-green-50/90 border border-green-200/60 text-green-800 text-sm" role="alert">
            <div class="flex items-start gap-2">
                <svg class="w-5 h-5 flex-shrink-0 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    @if($currentOrg = auth()->user()->currentOrganization())
        <div class="flex justify-between items-center p-4 bg-gradient-to-r from-indigo-50 to-white rounded-xl border border-indigo-100/50 shadow-sm" role="status" aria-live="polite">
            ...
            @can('update', $currentOrg)
                <a href="{{ route('dashboard.organization-settings.edit') }}" class="inline-flex items-center justify-center min-h-[44px] px-4 py-2.5 text-sm font-medium text-indigo-600 hover:text-indigo-800 hover:bg-indigo-100 rounded-lg transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2" aria-label="{{ __('Settings') }}">
                    {{ __('Settings') }}
                </a>
            @endcan
        </div>
    ...
    <div class="flex justify-start">
        <a href="{{ route('organizations.create') }}" class="inline-flex items-center justify-center gap-2 min-h-[44px] px-4 py-2.5 bg-indigo-600 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 transition-colors duration-200" aria-label="{{ __('Create New Organization') }}">
                        ...
                        <form action="{{ route('organizations.switch', $org) }}" method="POST" class="block" role="listitem">
                            ...
                            <button type="submit" class="w-full text-start rounded-xl border-2 border-gray-200/60 bg-gradient-to-br from-white to-gray-50/30 hover:border-indigo-300 hover:from-indigo-50 hover:to-white focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-indigo-500/30 transition-all duration-200 ease-out min-h-[44px] p-4 flex items-center justify-between gap-4 cursor-pointer shadow-sm hover:shadow-md group relative" aria-label="{{ __('Switch to organization', ['name' => $org->name]) }}">
                                ...
                                @if($currentOrg && $currentOrg->id === $org->id)
                                    <span class="absolute inset-0 rounded-xl border-2 border-indigo-500 bg-indigo-50/90 pointer-events-none flex items-center justify-end pe-4" aria-hidden="true">
                ...
                @empty
                    <li class="py-10 text-center text-sm text-gray-500 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50/30" role="status">
                        <div class="space-y-2">
                            ...
                            <a href="{{ route('organizations.create') }}" class="inline-flex items-center justify-center gap-1 min-h-[44px] px-4 py-2.5 text-indigo-600 hover:text-indigo-800 font-medium rounded-lg transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2" aria-label="{{ __('Create one') }}">
                                {{ __('Create one') }}
                            </a>
                        </div>
                    </li>
