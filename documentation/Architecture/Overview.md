...
**File**: `app/Services/SystemAuditLogger.php`  
**Model**: `App\Models\SystemAuditLog`

Records every significant system action with full context:

```php
SystemAuditLogger::log(
    actor: $user,           // nullable — system events have no actor
    action: 'organization.subscriptions_synced',
    target: $organization,  // Eloquent model or int ID
    metadata: ['key' => 'value'],
)
```

Stored fields: `actor_id`, `target_type`, `target_id`, `action`, `metadata` (JSON), `ip_address`, `user_agent`.

Used for: billing events, org membership changes, impersonation sessions, subscription sync, product engine operations.
...
| Topic | Document |
|-------|---------|
| Term definitions | [[Architecture/Glossary]] |
| Auth & passkeys | [[Architecture/Auth]] |
| Event state machine | [[Architecture/EventLifecycle]] |
| Queue & async jobs | [[Architecture/AsyncQueue]] |
| Full diagram index | [[Architecture/README]] |
